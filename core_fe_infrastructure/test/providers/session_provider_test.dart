...
MockNoSqlStorageManager _mockCachedINoSqlStorageManager = MockNoSqlStorageManager();
...
void main() {
  ...
  test('valid session start', () async {
    when(
      _mockNoSqlStorageManager.addOrUpdate(
          key: StorageKey.currentUserFolder, data: userSession, expiryDate: userSession.expiryDate, shared: true),
  ...
  test('end session', () async {
    when(_mockNoSqlStorageManager.delete(StorageKey.currentUserFolder, shared: true))
        .thenAnswer((realInvocation) => Future.value());
    when(_mockCachedINoSqlStorageManager.delete(StorageKey.currentUserFolder, shared: true))
        .thenAnswer((realInvocation) => Future.value());

    await _sessionProvider.endSession();

    verify(_mockNoSqlStorageManager.delete(StorageKey.currentUserFolder, shared: true));
    verify(_mockCachedINoSqlStorageManager.delete(StorageKey.currentUserFolder, shared: true));
  });
  ...
  test('get current sessoin from cached storage', () async {
    ...
    verify(
      _mockCachedINoSqlStorageManager.get<UserSession>(StorageKey.currentUserFolder, shared: true, ignoreExpiry: true),
    );
    verifyNever(
      _mockNoSqlStorageManager.get<UserSession>(StorageKey.currentUserFolder, shared: true, ignoreExpiry: true),
    );
  ...
  test('get current sessoin from disk storage', () async {
    ...
    when(_mockNoSqlStorageManager.get<UserSession>(StorageKey.currentUserFolder, shared: true, ignoreExpiry: true))
        .thenAnswer((realInvocation) => Future.value(userSession));
    var currentSession = await _sessionProvider.getCurrentSession();
