...
void main() {
  ...
  final networkManager = HttpNetworkManagerImpl(mockNetworkProvider, mockHttpHelper, mockConnectivity);
  final headers = {'h1': 'v1', 'h2': 'v2'};
  final defaultHeaders = {'dh1': 'dv1', 'dh2': 'dv2'};
  setUpAll(() {
    when(() => mockHttpHelper.getDefaultHeaders()).thenAnswer((realInvocation) async => defaultHeaders);
    when(() => mockConnectivity.isConnected()).thenAnswer((realInvocation) async => true);
  ...
  test('Expected connection exception when no internet connection', () async {
    when(mockConnectivity.isConnected).thenAnswer((realInvocation) async => false);

    expect(
      () async => await networkManager.get<Todo>(
        request: GetRequest(url: 'https://jsonplaceholder.typicode.com/posts/1'),
  ...
  group('updateRequestOptions', () {
    test('expected DefaultOptions when passed options is null', () async {
      ...
      var expectedOptions = RequestOptions(headers: defaultHeaders, contentType: ContentType.json);
    ...
    test('expect new requestOptions with (new+old) headers when add new headers', () async {
      final options = RequestOptions(headers: headers);

      var updatedOptions = await networkManager.updateRequestOptions(options);

      var expectedHeaders = {'h1': 'v1', 'h2': 'v2', 'dh1': 'dv1', 'dh2': 'dv2'};
...
//todo: Mock
  ...
  group('get', () {
    test('valid GET request with valid 200 valid response', () async {
      ...
      final responseOptions = ResponseOptions(responseType: ResponseType.json, fromJson: (json) => Todo.fromJson(json));
      final httpResponse = HttpResponse<Todo>(data: todo, statusCode: HttpStatus.ok, statusMessage: ok);
      when(mockConnectivity.isConnected).thenAnswer((realInvocation) async => true);
      registerFallbackValue(getRequest);
      registerFallbackValue(responseOptions);
      registerFallbackValue(await networkManager.updateRequestOptions(null));
      when(
  ...
  group('POST', () {
    test('valid POST request with valid 200 valid response', () async {
      const todo = Todo(
          ...
          body:
              'quia et suscipit\nsuscipit recusandae consequuntur expedita et cum\nreprehenderit molestiae ut ut quas totam\nnostrum rerum est autem sunt rem eveniet architecto');
      var postRequest = PostRequest(url: 'https://jsonplaceholder.typicode.com/posts/1', body: todo);
      final httpResponse = HttpResponse<Todo>(data: todo, statusCode: HttpStatus.ok, statusMessage: ok);
      var responseOptions = ResponseOptions(fromJson: (json) => Todo.fromJson(json));
      when(
        () => mockNetworkProvider.post<Todo>(
          request: any(named: 'request')!,
          requestOptions: any(named: 'requestOptions'),
          responseOptions: any(named: 'responseOptions'),
        ),
      ).thenAnswer((realInvocation) async => httpResponse);
  ...
  group('PUT', () {
    test('valid put request with valid 200 valid response', () async {
      const todo = Todo(
          ...
          body:
              'quia et suscipit\nsuscipit recusandae consequuntur expedita et cum\nreprehenderit molestiae ut ut quas totam\nnostrum rerum est autem sunt rem eveniet architecto');
      var putRequest = PutRequest(url: 'https://jsonplaceholder.typicode.com/posts/2', body: todo.toJson());
      final httpResponse = HttpResponse<Todo>(data: todo, statusCode: HttpStatus.ok, statusMessage: ok);
  ...
  test('Download files request with valid 200 valid response', () async {
    var putRequest = DownloadFileRequest(
        url: 'https://cdn.iconscout.com/icon/free/png-512/flutter-2038877-1720090.png', savePath: imagePath);
    final requestOptions = RequestOptions();
    final httpResponse = HttpResponse<void>(data: null, statusCode: HttpStatus.ok, statusMessage: ok);
